...
def create_virtualenv(venv_path: Union[str, Path] = VENV_NAME) -> None:
    ...
    run_command([str(venv_path / 'bin' / 'pip'), 'install', '-q', 'nbformat', 'nbconvert', 'ipykernel'])


def execute_notebook(notebook_path: Union[str, Path]) -> None:
    """Exécute un notebook Jupyter en utilisant nbconvert et vérifie l'absence d'erreurs."""
    ...
    try:
        ...
        raise RuntimeError(f"Erreur lors de l'exécution du notebook {notebook_path}: {e}")
