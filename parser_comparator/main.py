...
def compare_parsing(experiment_name : str):
    ...
    for root, dirs, files in os.walk(sources_dir_path):
        for dir_name in dirs:
            root=Path(root)
            doc_db_source_path = root / dir_name

            # Create the documentary database results directory if it does not exist
            doc_db_results_path = results_dir_path / dir_name
            os.makedirs(doc_db_results_path, exist_ok=True)

            experiment_path = doc_db_results_path / experiment_name
            ...
            for pdf_filename in glob( "*.pdf",root_dir=doc_db_source_path):

                print(f'processing {pdf_filename}... ')
                # Create a sub directory to store the parsings of the experiment
                sub_dir_pdf_path = experiment_path / pdf_filename
                os.makedirs(sub_dir_pdf_path)


                # Load the pdf file
                pdf_path = doc_db_source_path / pdf_filename
                ...
                if pdf_multi_loader.parser.debug :
                    ...
                    sub_sub_parsings_dir = sub_dir_pdf_path / 'parsings_by_parser'
                    sub_sub_parsings_dir.mkdir()
                    ...
                    parser_name2concatenated_parsed_docs = {parser_data[0]:
                                                                _default_page_delimitor.join([doc.page_content for doc in parser_data[1][0]]) for parser_data in parsers_result}
                    ...
                    for parser_name, concatenated_docs in parser_name2concatenated_parsed_docs.items():
                        output_file_path = os.path.join(sub_sub_parsings_dir,
                                                        f"{pdf_filename}_parsed_{parser_name}.txt")
